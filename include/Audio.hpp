    ...
    try {
        ...
        while (avail > 0 && !audio->closed) {
            if (!audio->residual) {
                ...
                if (audio->pyAudioCallback) audio->pyAudioCallback(f, audio->reader->uri);
                if (audio->progressCallback) audio->update_progress(f.pts());
...
Audio::Audio(Reader* reader, Queue<Frame>* frames, int audio_driver_index) : reader(reader), frames(frames), audio_driver_index(audio_driver_index) {
    ...
    std::cout << "AUDIO DEVICE ID: " << device_id << std::endl;

    SDL_PauseAudioDevice(device_id, 0);
}

Audio::~Audio() {
    std::cout << "audio destructor start" << std::endl;
    ...
    if (temp) free(temp);
    std::cout << "audio destructor finish" << std::endl;
}
