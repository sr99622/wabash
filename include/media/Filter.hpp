...
public:
    ...
    Filter(Decoder* decoder, const std::string& description, Queue<Frame>* input=nullptr, Queue<Frame>* output=nullptr) 
    ...
    void RawFilter(AVFrame* src, Frame& dest) {
        ex.ck(av_buffersrc_add_frame_flags(src_ctx, src, AV_BUFFERSRC_FLAG_KEEP_REF), ABAFF);

        int ret = -1;
        while ((ret = av_buffersink_get_frame(sink_ctx, av_frame)) >= 0) {
            dest = std::move(Frame(av_frame));
        }
        if (ret != AVERROR(EAGAIN) && ret != AVERROR_EOF)
            ex.ck(ret, "error during filtering");
    }

    void GetFrame(const Packet& pkt, Frame& dest) {
        try {
            int ret = -1;
            ex.ck((ret = avcodec_send_packet(decoder->codec_ctx, pkt.pkt)), ASP);
            while ((ret = avcodec_receive_frame(decoder->codec_ctx, decoder->av_frame)) >= 0) {
                if (decoder->av_frame->format == hw_pix_fmt) {
                    ex.ck(av_hwframe_transfer_data(decoder->sw_frame, decoder->av_frame, 0), AHTD);
                	ex.ck(av_frame_copy_props(decoder->sw_frame, decoder->av_frame), AFCP);
                    Frame term(decoder->av_frame);
                    RawFilter(decoder->sw_frame, dest);
                }
                else {
                    RawFilter(decoder->av_frame, dest);
                }
            }
            if (ret != AVERROR(EAGAIN) && ret != AVERROR_EOF)
            ...
            std::stringstream str;
            str << decoder->str_media_type << " decode exception: " << e.what() << std::endl;
