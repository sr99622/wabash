...
public:
    ...
    void open(const std::string& base_filename /*, std::map<std::string, std::string>& metadata*/) {
        ...
        if (reader->video_stream_index >= 0 && !disable_video) {
            ...
            std::cout << "VIDEO STREAM INDEX: " << video_stream->index << std::endl;
        }
        if (reader->audio_stream_index >= 0 && !disable_audio) {
            ...
            std::cout << "AUDIO STREAM INDEX: " << audio_stream->index << std::endl;
        }
    ...
    int write() {
        ...
        if (pkt.is_null()) {
            std::cout << reader->uri << " writer recvd null pkt" << std::endl;
            return 0;
        }
